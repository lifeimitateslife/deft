...
WindowServer skips fully transparent pixels. While native blur is active, a constant 1/255 black background alpha enables the filter. That seed is identical at all nonzero strengths; the slider changes the native radius, not this alpha. Zero blur restores a fully clear base. The Mac controller invalidates the transparent-window shadow after material updates so its cached shadow does not darken the clear backdrop on macOS 15. The window keeps its normal shadow. Foreground/window opacity remains 1. The old HUD vibrancy tint is replaced by the adjustable effect on supported Macs; the existing background-fill slider still controls opacity.
